...
partial def evalPacket : Packet → Nat
| (Literal _ val) => val
| (Container _ 0 xs) => (xs.map evalPacket).foldr (·+·) 0
| (Container _ 1 xs) => (xs.map evalPacket).foldr (·*·) 1
| (Container _ 2 xs) => (xs.map evalPacket).foldr (min · ·) 100000000000000000000
| (Container _ 3 xs) => (xs.map evalPacket).foldr (max · ·) 0
| (Container _ 5 xs) => let ps := (xs.map evalPacket)
                        if ps.get! 0 > ps.get! 1 then 1 else 0
| (Container _ 6 xs) => let ps := (xs.map evalPacket)
                        if ps.get! 0 < ps.get! 1 then 1 else 0
| (Container _ 7 xs) => let ps := (xs.map evalPacket)
                        if ps.get! 0 == ps.get! 1 then 1 else 0
| _ => 0
...
do
  ...
  let encoded := input[0]
  IO.println $ evalPacket $ decode encoded
