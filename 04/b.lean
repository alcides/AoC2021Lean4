...
def winnerAndLosers : List Board → List Board × List Board
| [] => ([], [])
| (b :: bs) =>
    let (ws, ls) := winnerAndLosers bs
    if isWinner b then (b::ws, ls) else (ws, b::ls)


def findLastWinner (boards: List Board) : List Int → Int × Int
| [] => (-1, -1)
| (n :: ns) => do
    let nboards := boards.map (playNumber n)
    match winnerAndLosers nboards with
      | ([b], []) => (sumUnmarked b, n)
      | (_, ls) => findLastWinner ls ns
...
do let input ← IO.FS.lines "input.txt"
   ...
   match lines with
    ...
    | (draws_raw :: boards_raw) => do
      ...
      let (totalBoard, finalNumber) := findLastWinner boards numbers
