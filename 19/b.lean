...
partial def computeOffsets (acc: Array (Int × Int × Int)) : List (Array (Int × Int × Int)) → List (Int × Int × Int)
| [] => [(0,0,0)]
| (x :: ys) => match findOverLap acc x with
    | none => dbgTrace [ys.length + 1].toString $ fun _ => computeOffsets acc (ys ++ [x])
    | some (offset, b) => offset::computeOffsets b ys
...
do
  ...
  let offsets := computeOffsets scanners[0] scanners.toList.tail!

  let mut m := 0
  for (x1, x2, x3) in offsets do
    for (y1, y2, y3) in offsets do
      let d := (abs (x1-y1)) + (abs (x2-y2)) + abs (x3-y3)
      m := max m d
