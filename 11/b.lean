...
partial def iterate (matrix: Array (Array Nat)) (positions: Array (Nat × Nat)) (acc:Nat) : Nat × Array (Array Nat) × Bool := do
  let mut m := matrix
  let mut f := acc
  let mut npos := #[]
  for i in [:10] do
  ...
  --dbgTrace ( (npos.size.toSuperscriptString) ++ " " ++ (positions.size.toSuperscriptString) ) $ fun _ =>
  if npos.size + positions.size == 100 then 
      (f, m, true)
  else if npos.size > 0 then
